...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['investor'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<InvestorInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/investors` : null),
    () => getInvestors({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Investors',
      path: isTenantUser ? `/investors/view/${data?.[0]?.id}` : '/investors',
      entity: 'investor',
      service: AccessServiceEnum.PROJECT,
      icon: FiDollarSign,
    },
    { name: 'Paintings', path: '/paintings', entity: 'painting', service: AccessServiceEnum.PROJECT, icon: FiImage },
    {
      name: 'Purchases',
      path: '/purchases',
      entity: 'purchase',
